...
$MotionPath = Join-Path $Root "src/main/java/net/droingo/decor/client/animation/BeastSkullJawMotionState.java"
$SeatPath = "src/main/java/net/droingo/decor/entity/BeastSkullSeatEntity.java"

if (-not (Test-Path -LiteralPath $MotionPath)) {
    throw "Could not find BeastSkullJawMotionState.java. Run this from the Droingo's Decor project root."
}
...
# Reverse only the sublevel inertia direction.
$MotionText = [System.IO.File]::ReadAllText($MotionPath)

$OldTarget = "(float) (-acceleration.dot(forward) * INERTIA)"
$NewTarget = "(float) (acceleration.dot(forward) * INERTIA)"

if (-not $MotionText.Contains($OldTarget)) {
    throw "Could not find the expected jaw inertia expression in BeastSkullJawMotionState.java."
}

$MotionText = $MotionText.Replace($OldTarget, $NewTarget)
[System.IO.File]::WriteAllText($MotionPath, $MotionText, $Utf8NoBom)
...
Write-Host ""
Write-Host "Reversed jaw inertia and repaired mouth-seat crawl/free-look behavior."
Write-Host "Building..."
Write-Host ""
