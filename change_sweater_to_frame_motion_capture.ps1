...
$BackupRoot = Join-Path $ProjectRoot (".frame_motion_capture_backup_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
Write-Utf8NoBom `
    "src/main/java/net/droingo/decor/client/animation/HangingGravityMotionState.java" `
@'
...
Write-Utf8NoBom `
    "src/main/java/net/droingo/decor/client/render/WallDecorRenderer.java" `
@'
...
Write-Host ""
Write-Host "Installed frame-based sweater motion capture."
...
Write-Host ""
Write-Host "Test the sweater in the affected world first."
Write-Host "The bobblehead files were not present in the uploaded ZIP, so this patch"
Write-Host "changes only the sweater motion path for a clean A/B test."
