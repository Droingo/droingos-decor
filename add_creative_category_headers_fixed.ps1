...
# The previous header patch wrote part of its own PowerShell source into
# DecorItems.java because its here-string terminators were indented.
# Restore the newest backup made by that failed run before applying the fix.
$ProjectRoot = (Get-Location).Path
$FailedBackup = Get-ChildItem -LiteralPath $ProjectRoot -Directory |
    Where-Object { $_.Name -like ".decor_header_backup_*" } |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

if ($null -ne $FailedBackup) {
    Write-Host "Restoring files from failed patch backup:"
    Write-Host $FailedBackup.FullName

    Get-ChildItem -LiteralPath $FailedBackup.FullName -Recurse -File |
        ForEach-Object {
            $relative = $_.FullName.Substring($FailedBackup.FullName.Length)
